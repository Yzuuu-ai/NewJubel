...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const [
      ...
      // Total transaksi (only paid transactions)
      prisma.transaksi.count({
        where: {
          status: {
            in: ['DIBAYAR_SMARTCONTRACT', 'MENUNGGU_KIRIM_AKUN', 'DIKIRIM', 'DIKONFIRMASI_PEMBELI', 'SENGKETA', 'SELESAI']
          }
        }
      }),
      // Total sengketa
      ...
      // Users created today (exclude admin)
      prisma.user.count({
        where: {
          dibuatPada: {
            gte: startOfToday,
      ...
      // Transactions created today (only paid transactions)
      prisma.transaksi.count({
        where: {
          dibuatPada: {
            gte: startOfToday,
